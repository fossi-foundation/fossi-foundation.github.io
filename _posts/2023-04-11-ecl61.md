...
Other improvements in the new release include support for POP0420 implicit namespace packages, the ability to avoid unnecessary rebuilds "in most cases" using the new flow API, and support for single compilation unit compiles in the _modelsim_ backend, multiple top levels in the _icarus_ backend, and Verilog output with simulation file tag recognition in the _yosys_ and _vivado_ backends.  
